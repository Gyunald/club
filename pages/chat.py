...
# db.collection('notice').document((datetime.utcnow()+timedelta(hours=9)).strftime('%Y.%m.%d')).update({'공지':[]})

collection = db.collection('notice').document((datetime.utcnow()+timedelta(hours=9)).strftime('%Y.%m.%d'))
notice = collection.get().to_dict()
notice_list = notice.get('공지')
if nickname :
    if st.session_state.club != '' :
        ...
        with c[1]:
            with st.form("notice",clear_on_submit=True):
                ...
                if submitted :
                    if t2 != '':
                        notice_list.append(f"🔥 {(datetime.utcnow()+timedelta(hours=9)).strftime('%m.%d-%H:%S')} \n{nickname} : {t2}")
                        collection.update(notice)
                        st.experimental_rerun()
            ...
            with c[i]:
                with st.form(' '+j):
                    if doc_ref.id == '배드민턴' :
                        ...
                        emoji = '🎲'
                    st.write(f"##### {doc_document.get('날짜')} {emoji} {doc_document.get('시간')}")
