    ...
    for i,j in zip(range(len(c)), sorted(doc.keys(),reverse=True)):
        ...
        with c[i]:
            with st.form(j):
                ...
                st.warning('[🚗 카카오맵](%s)' % 'https://map.kakao.com/?map_type=TYPE_MAP&target=car&rt=,,&rt1=&rt2=%ED%8C%8C%EC%A3%BC%EC%8B%9C%EB%B0%B0%EB%93%9C%EB%AF%BC%ED%84%B4%EC%A0%84%EC%9A%A9%EA%B5%AC%EC%9E%A5&rtIds=,&rtTypes=')
